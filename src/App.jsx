...
export default function App() {
  ...
  const [messages, setMessages] = useState([
    { role: "bot", text: "Soy Morfis Elemental. Pedime transformaciones en lenguaje natural: «gírala un poco a la izquierda y hacela más chica», «pasala a blanco y negro», «resaltá la esquina de abajo a la derecha». Encadeno las operaciones y te muestro cada matriz y la resultante." },
  ]);
  ...
  const tintedImage = useCallback(() => {
    ...
    if (colorChain.length) {
      ...
      for (let i = 0; i < p.length; i += 4) { const [r, g, b] = applyColorChain([p[i], p[i + 1], p[i + 2]], colorChain); p[i] = r; p[i + 1] = g; p[i + 2] = b; }
  ...
  function handleIntent(it) {
    if (!it) return botSay("No entendí. Podés pedir geometría (rotar, escalar, reflejar, sesgar, trasladar), colores (grises, negativo, sepia, saturación, canales) o resaltar una zona.");
    if (it.space === "control") {
    ...
    if (it.space === "geo") {
      ...
      return botSay(`Agregué: ${ex.title}.${geoCount >= 1 ? " Mirá la matriz resultante a la derecha." : ""}`);
    }
  }
  ...
  const chips = mode === "imagen"
    ? [["Rótala 30°", RotateCw], ["Trasládala 2 a la derecha", Move], ["Agrándala 1.4x", Grid3x3], ["Escala de grises", Palette], ["Resaltá el centro", Grid3x3]]
    ...
    <div className="root">
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        .send { background:${J}; color:#04121f; border:none; border-radius:11px; padding:0 14px; cursor:pointer; display:flex; align-items:center; }
      ...
      <div className="wrap">
        <div className="head">
          <div className="logo">Morfis <span className="el">Elemental</span></div>
        </div>
        ...
        <div className="grid">
          ...
          <div className="right">
            <div className="card pad">
              ...
              {info ? <div className="title">{info.title}</div> : <div className="empty">Pedí una transformación para ver su matriz.</div>}
            ...
            <div className="card chat">
              ...
              <div className="composer">
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onSend()} placeholder="Escribí una transformación…" />
